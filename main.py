...
def getData(backtest: BackTest):
    length = 0
    data = {}
    
    ticker_data = yf.Ticker(backtest.safe)
    ticker_df = ticker_data.history(start=backtest.start, end=backtest.end, interval=backtest.interval)
    data[backtest.safe] = ticker_df
    length = len(ticker_df)
    
    for asset in backtest.assets:
        ticker_data = yf.Ticker(asset)
        ticker_df = ticker_data.history(start=backtest.start, end=backtest.end, interval=backtest.interval)
        data[asset] = ticker_df
        length = min(length, len(ticker_df))
        
    print(length)
    return data, length

def getLastPrice(data, symbol, timelaps):
...
def getTotalValue(backtest: BackTest, qty, ):
    total_value = 0
    total_value += backtest.cash
    twst = 0
    for symbol in backtest.assets:
        total_value += getLastPrice() * qty[symbol]
